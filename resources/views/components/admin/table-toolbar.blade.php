        ...
        <div class="aj-result-meta">
            ...
            <div class="aj-view-toggle" style="display:inline-flex;padding:4px;border-radius:14px;border:1px solid var(--ajp-line);background:#fff;">
                @foreach($viewModes as $mode)
                    <button type="button" data-view="{{ $mode['value'] }}" class="{{ ($mode['active'] ?? false) ? 'is-active' : '' }}" style="min-width:42px;height:38px;border:0;border-radius:10px;background:transparent;color:#516278;font-weight:900;">
                        <i class="{{ $mode['icon'] }}"></i>
                    </button>
                @endforeach
            </div>
