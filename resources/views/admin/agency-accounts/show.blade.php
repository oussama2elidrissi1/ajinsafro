...
<div class="aj-page-head" style="margin-bottom:18px;">
    <div>
        <h1>{{ $account->name }}</h1>
        <p>{{ $account->email }} â€¢ {{ $account->branch?->display_name ?? 'Aucun point de vente' }}</p>
    </div>
    <div style="display:flex;gap:10px;flex-wrap:wrap;">
        @if(Route::has('admin.agency-accounts.edit'))
            <a href="{{ route('admin.agency-accounts.edit', $account) }}" class="aj-btn primary">Ã‰diter</a>
        @endif
    ...
    <div class="col-lg-4">
        <div class="aj-card">
            <div class="aj-card-body text-center">
                ...
                <div class="aj-subtle">{{ $account->roles->first()?->name ?? 'Sans rÃ´le' }}</div>
        ...
        <div class="aj-card mt-3">
            <div class="aj-card-body">
                ...
                <div class="aj-subtle">EmployÃ© liÃ© : {{ $account->agencyEmployee?->full_name ?? 'Aucun' }}</div>
                <div class="aj-subtle">Fonction : {{ $account->job_title ?? 'â€”' }}</div>
                <div class="aj-subtle">TÃ©lÃ©phone : {{ $account->phone ?? 'â€”' }}</div>
                <div class="aj-subtle">DerniÃ¨re connexion : {{ $account->last_login_at?->timezone('Africa/Casablanca')?->format('d/m/Y H:i') ?? 'Jamais' }}</div>
                <div class="aj-subtle">RÃ©servations affectÃ©es : {{ (int) ($account->assigned_reservations_count ?? 0) }}</div>
            </div>
        </div>
