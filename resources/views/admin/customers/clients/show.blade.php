...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-flex align-items-center justify-content-between">
                <h4 class="page-title mb-0 font-size-18">Fiche client â€“ {{ $client->full_name }}</h4>
    ...
    <div class="row mb-3">
        <div class="col-12">
            ...
            <a href="{{ route('admin.customers.clients.index') }}" class="btn btn-outline-secondary btn-sm">Retour Ã  la liste</a>
        ...
        <div class="col-lg-4">
                ...
                <div class="card-body">
                    ...
                    <p class="mb-1"><strong>Type :</strong>
                        <span class="badge {{ $client->client_type === 'company' ? 'bg-info' : ($client->client_type === 'agency' ? 'bg-secondary' : 'bg-light text-dark') }}">
                            {{ $client->client_type === 'individual' ? 'Particulier' : ($client->client_type === 'company' ? 'SociÃ©tÃ©' : 'Agence') }}
                        </span>
                    </p>
                    ...
                    @if($client->assignedTo)
                        <p class="mb-1"><strong>AssignÃ© Ã  :</strong> {{ $client->assignedTo->name }}</p>
                    @endif
                    <p class="mb-0 small text-muted">CrÃ©Ã© le {{ $client->created_at->format('d/m/Y H:i') }}
                        @if($client->updated_at && $client->updated_at != $client->created_at)
                            Â· ModifiÃ© le {{ $client->updated_at->format('d/m/Y H:i') }}
                        @endif
                    </p>
                </div>
            </div>
            ...
            <div class="card mt-3">
                <div class="card-body">
                    ...
                    @if($client->phone)<p class="mb-1">TÃ©l : {{ $client->phone }}</p>@endif
        ...
        <div class="col-lg-8">
            <div class="card mb-3">
                <div class="card-body">
                    ...
                    <p class="mb-1"><strong>Passeport :</strong> {{ $client->passport_number ?? 'â€”' }} @if($client->passport_expiry_date) (exp. {{ $client->passport_expiry_date->format('d/m/Y') }}) @endif</p>
                    <p class="mb-1"><strong>CIN / ID :</strong> {{ $client->national_id_number ?? 'â€”' }}</p>
                    <p class="mb-0"><strong>Visa :</strong> {{ $client->visa_required ? 'Requis â€“ ' . $client->visa_status : 'Non requis' }}</p>
                </div>
            </div>
            ...
            <div class="card mb-3">
                <div class="card-body">
                    ...
                    <p class="mb-1"><strong>CatÃ©gorie :</strong> {{ $client->traveler_category ?? 'â€”' }}</p>
                    <p class="mb-1"><strong>Destination prÃ©fÃ©rÃ©e :</strong> {{ $client->preferred_destination ?? 'â€”' }}</p>
                    <p class="mb-1"><strong>Budget :</strong> {{ $client->budget_display ?? 'â€”' }}</p>
                    @if($client->special_requests)
                        <p class="mb-0"><strong>Demandes spÃ©ciales :</strong><br>{{ $client->special_requests }}</p>
                    @endif
                </div>
            </div>
            ...
            @if($client->company_name || $client->billing_name)
                <div class="card mb-3">
                    <div class="card-body">
                        ...
                        @if($client->company_name)<p class="mb-1"><strong>SociÃ©tÃ© :</strong> {{ $client->company_name }}</p>@endif
            ...
            {{-- Emplacements futurs : rÃ©servations, devis, paiements, documents --}}
