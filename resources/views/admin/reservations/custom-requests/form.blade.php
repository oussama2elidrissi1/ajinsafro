...
<form method="POST" action="{{ $formAction }}" class="crq-page" id="crq-form">
    @csrf
    @if($formMethod !== 'POST')
        @method($formMethod)
    @endif
    ...
    <div class="crq-hero">
        <div>
            <h2>{{ $isEdit ? 'Modifier la demande' : 'Créer une demande à la carte' }}</h2>
            <p>{{ $isEdit ? $customRequest->reference : 'Workflow séparé des réservations standard, sans voyage obligatoire.' }}</p>
        </div>
        <a href="{{ route('admin.reservations.custom-requests.index') }}" class="crq-btn crq-btn-soft"><i class="bx bx-list-ul"></i> Liste des demandes</a>
    </div>
    ...
    <div class="crq-shell">
            ...
            <div class="crq-grid">
                <div class="crq-field">
                    ...
                    <select name="client_type" class="crq-select">
                        @foreach(['particular' => 'Particulier', 'company' => 'Entreprise', 'agency' => 'Agence'] as $value => $label)
                            <option value="{{ $value }}" @selected(old('client_type', $customRequest->client_type) === $value)>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="crq-field">
                    ...
                    <select name="client_gender" class="crq-select">
                        ...
                        <option value="M" @selected(old('client_gender', $customRequest->client_gender) === 'M')>M.</option>
                        <option value="F" @selected(old('client_gender', $customRequest->client_gender) === 'F')>Mme</option>
                    </select>
                </div>
                <div class="crq-field">
                    <label class="crq-label">Nom complet <span class="crq-required"></span></label>
                    <input name="client_name" class="crq-input" value="{{ old('client_name', $customRequest->client_name) }}" required>
                    @error('client_name')<span class="crq-error">{{ $message }}</span>@enderror
                </div>
                <div class="crq-field">
                    <label class="crq-label">Téléphone <span class="crq-required"></span></label>
                    <input name="client_phone" class="crq-input" value="{{ old('client_phone', $customRequest->client_phone) }}" required>
                    @error('client_phone')<span class="crq-error">{{ $message }}</span>@enderror
                </div>
                <div class="crq-field">
                    <label class="crq-label">WhatsApp</label>
                    <input name="client_whatsapp" id="client_whatsapp" class="crq-input" value="{{ old('client_whatsapp', $customRequest->client_whatsapp) }}">
                    <label class="crq-check mt-1"><input type="checkbox" name="whatsapp_same_as_phone" id="whatsapp_same_as_phone" value="1" @checked(old('whatsapp_same_as_phone', $customRequest->whatsapp_same_as_phone))> WhatsApp = téléphone</label>
                </div>
                <div class="crq-field">
                    <label class="crq-label">Email</label>
                    <input type="email" name="client_email" class="crq-input" value="{{ old('client_email', $customRequest->client_email) }}">
                    @error('client_email')<span class="crq-error">{{ $message }}</span>@enderror
                </div>
                <div class="crq-field full">
                    <label class="crq-label">Canal préféré</label>
                    <div class="crq-checks">
                        @foreach(['call' => 'Appel', 'whatsapp' => 'WhatsApp', 'email' => 'Email'] as $value => $label)
                            <label class="crq-check"><input type="checkbox" name="preferred_channels[]" value="{{ $value }}" @checked(in_array($value, $channels, true))> {{ $label }}</label>
                        @endforeach
                    </div>
                </div>
            </div>
        ...
        <section class="crq-pane" data-crq-pane="2">
            <div class="crq-grid-3">
                <div class="crq-field">
                    ...
                    <input type="number" min="1" name="adults" class="crq-input" value="{{ old('adults', $customRequest->adults ?: 1) }}">
                    @error('adults')<span class="crq-error">{{ $message }}</span>@enderror
                </div>
                <div class="crq-field">
                    <label class="crq-label">Ville de départ</label>
                    <input name="departure_city_text" class="crq-input" value="{{ old('departure_city_text', $customRequest->departure_city_text) }}">
                </div>
                <div class="crq-field">
                    <label class="crq-label">Destination</label>
                    <input name="destination_text" class="crq-input" value="{{ old('destination_text', $customRequest->destination_text) }}">
                    @error('destination_text')<span class="crq-error">{{ $message }}</span>@enderror
                </div>
                <div class="crq-field">
                    <label class="crq-label">Date départ</label>
                    <input type="date" name="departure_date" class="crq-input" value="{{ old('departure_date', optional($customRequest->departure_date)->toDateString()) }}">
                </div>
                <div class="crq-field">
                    <label class="crq-label">Date retour</label>
                    <input type="date" name="return_date" class="crq-input" value="{{ old('return_date', optional($customRequest->return_date)->toDateString()) }}">
                    @error('return_date')<span class="crq-error">{{ $message }}</span>@enderror
                </div>
                <div class="crq-field">
                    <label class="crq-label">Devise</label>
                    <input name="currency" class="crq-input" value="{{ old('currency', $customRequest->currency ?: 'MAD') }}">
                </div>
                <div class="crq-field">
                    <label class="crq-label">Budget min</label>
                    <input type="number" step="0.01" name="budget_min" class="crq-input" value="{{ old('budget_min', $customRequest->budget_min) }}">
                </div>
                <div class="crq-field">
                    <label class="crq-label">Budget max</label>
                    <input type="number" step="0.01" name="budget_max" class="crq-input" value="{{ old('budget_max', $customRequest->budget_max) }}">
                    @error('budget_max')<span class="crq-error">{{ $message }}</span>@enderror
                </div>
                <div class="crq-field">
                    <label class="crq-label">Flexibilité</label>
                    <label class="crq-check"><input type="checkbox" name="flexible_dates" value="1" @checked(old('flexible_dates', $customRequest->flexible_dates))> Dates flexibles</label>
                </div>
                <div class="crq-field full">
                    ...
                    <div class="crq-row-list" data-repeat-list="children">
                        @foreach(($children ?: [[]]) as $idx => $child)
                            <div class="crq-row">
                                <input type="number" min="0" max="17" name="children[{{ $idx }}][age]" class="crq-input" placeholder="Age" value="{{ $child['age'] ?? '' }}">
                                <input type="date" name="children[{{ $idx }}][birth_date]" class="crq-input" value="{{ $child['birth_date'] ?? '' }}">
                ...
                <div class="crq-field full">
                    ...
                    <div class="crq-row-list" data-repeat-list="infants">
                        @foreach(($infants ?: [[]]) as $idx => $infant)
                            <div class="crq-row">
                                <input type="number" min="0" max="3" name="infants[{{ $idx }}][age]" class="crq-input" placeholder="Age" value="{{ $infant['age'] ?? '' }}">
                                <input type="date" name="infants[{{ $idx }}][birth_date]" class="crq-input" value="{{ $infant['birth_date'] ?? '' }}">
                                <button type="button" class="crq-remove" data-repeat-remove><i class="bx bx-trash"></i></button>
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="crq-field full">
                    ...
                    <textarea name="passengers_note" class="crq-textarea">{{ old('passengers_note', $customRequest->passengers_note) }}</textarea>
                </div>
            </div>
        </section>
        ...
        <section class="crq-pane" data-crq-pane="3">
            <div class="crq-service-grid">
                @foreach($serviceOptions as $key => $label)
                    @php $enabled = (bool) data_get($services, $key.'.enabled', array_key_exists($key, $services)); @endphp
                    <label class="crq-service {{ $enabled ? 'is-active' : '' }}" data-service-card="{{ $key }}">
                        <input type="checkbox" name="services[{{ $key }}][enabled]" value="1" class="d-none" @checked($enabled)>
                        <i class="bx {{ match($key) { 'flights' => 'bx-plane', 'accommodation' => 'bx-hotel', 'transfers' => 'bx-car', 'excursions' => 'bx-camera', 'omra' => 'bx-building-house', 'visa' => 'bx-id-card', 'insurance' => 'bx-shield-quarter', default => 'bx-dots-horizontal-rounded' } }}"></i>
                        {{ $label }}
                    </label>
                @endforeach
            </div>

            @include('admin.reservations.custom-requests.service-configs', ['services' => $services])
        </section>
                ...
                <div class="crq-field">
                    ...
                    <select name="priority" class="crq-select">
                        ...
                        @foreach($priorityOptions as $value => $label)
                            <option value="{{ $value }}" @selected(old('priority', $customRequest->priority) === $value)>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="crq-field">
                    ...
                    <select name="assigned_to" class="crq-select">
                        ...
                        @foreach($agents as $agent)
                            <option value="{{ $agent->id }}" @selected((int) old('assigned_to', $customRequest->assigned_to) === (int) $agent->id)>{{ $agent->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="crq-field">
                    ...
                    <select name="status" class="crq-select">
                        @foreach($statusOptions as $value => $label)
                            <option value="{{ $value }}" @selected(old('status', $customRequest->status ?: 'new') === $value)>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="crq-field">
                    ...
                    <select name="source" class="crq-select">
                        ...
                        @foreach($sourceOptions as $value => $label)
                            <option value="{{ $value }}" @selected(old('source', $customRequest->source ?: 'admin') === $value)>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="crq-field full">
                    <label class="crq-label">Notes client</label>
                    <textarea name="client_notes" class="crq-textarea">{{ old('client_notes', $customRequest->client_notes) }}</textarea>
                </div>
                <div class="crq-field full">
                    <label class="crq-label">Notes internes</label>
                    <textarea name="internal_notes" class="crq-textarea">{{ old('internal_notes', $customRequest->internal_notes) }}</textarea>
                </div>
                <div class="crq-field full">
                    <label class="crq-label">Réponse commerciale / devis</label>
                    <textarea name="admin_response" class="crq-textarea">{{ old('admin_response', $customRequest->admin_response) }}</textarea>
                </div>
                <div class="crq-field">
                    <label class="crq-label">Montant devis</label>
                    <input type="number" step="0.01" name="quoted_amount" class="crq-input" value="{{ old('quoted_amount', $customRequest->quoted_amount) }}">
                </div>
                <div class="crq-field full">
        ...
        <div class="crq-actions">
            ...
            <div>
                ...
                <button type="submit" name="submit_action" value="create" class="crq-btn crq-btn-primary">{{ $submitLabel }}</button>
