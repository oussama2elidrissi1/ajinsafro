    ...
    <div class="reservation-create__card">
        <div class="reservation-create__section-head">
            <div>
                ...
                <p class="reservation-create__section-subtitle">Identifiez un client existant ou créez une nouvelle fiche dans un flux propre et rapide.</p>
            </div>
        </div>
        ...
        <div id="existing-client-block" class="{{ $clientMode === 'existing' ? '' : 'd-none' }}">
            <div class="reservation-create__field">
                <label class="reservation-create__label" for="client_external_id">Client existant <span>*</span></label>
                <select name="client_external_id" id="client_external_id" class="reservation-create__input">
                    ...
                    @foreach($clients as $client)
                        <option value="{{ $client->id }}" {{ old('client_external_id') == $client->id ? 'selected' : '' }}>
                            [{{ $client->client_code }}] {{ $client->full_name }}@if($client->phone) — {{ $client->phone }}@endif
                        </option>
                    @endforeach
                </select>
            </div>
        ...
        <div id="new-client-block" class="{{ $clientMode === 'new' ? '' : 'd-none' }}">
            <div class="reservation-create__grid reservation-create__grid--two">
                ...
                <div class="reservation-create__field">
                    <label class="reservation-create__label" for="client_phone">Téléphone</label>
                ...
                <div class="reservation-create__field">
                    <label class="reservation-create__label" for="client_document_number">CIN / Passeport</label>
                ...
                <div class="reservation-create__field">
                    ...
                    <input type="text" name="client_document_type" id="client_document_type" class="reservation-create__input" value="{{ old('client_document_type') }}" placeholder="CIN, Passeport…">
                </div>
            </div>
        </div>
    </div>
