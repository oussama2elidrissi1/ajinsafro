...
<div class="fade-in max-w-[1680px] mx-auto pb-10">
    ...
    @if(config('app.debug') && isset($catalogMeta))
        {{-- Accès discret au debug (contenu complet dans le modal ci-dessous) --}}
        <div class="fixed bottom-5 right-5 z-[998]">
            <button type="button" id="ws-debug-modal-open" class="inline-flex items-center gap-1.5 rounded-lg border border-red-300/90 bg-red-50/95 px-3 py-1.5 text-[11px] font-bold uppercase tracking-wide text-red-700 shadow-md backdrop-blur-[1px] transition-colors hover:bg-red-100 hover:border-red-400" title="Ouvrir le panneau debug catalogue (APP_DEBUG)">
                <i class="fas fa-bug text-[10px] opacity-90"></i>
                Debug test
            </button>
        </div>
