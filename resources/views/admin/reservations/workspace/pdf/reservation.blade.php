<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="utf-8">
    <style>
        body { font-family: DejaVu Sans, sans-serif; font-size: 11px; color: #333; }
        h1 { font-size: 16px; color: #0e3a5a; }
        h2 { font-size: 13px; margin-top: 14px; color: #0083c4; }
        table { width: 100%; border-collapse: collapse; margin-top: 8px; }
        th, td { border: 1px solid #ddd; padding: 5px 7px; text-align: left; }
        th { background: #e6f3fa; }
    </style>
</head>
<body>
    <h1>Réservation #{{ $reservation->id }}</h1>
    <p><strong>Voyage :</strong> {{ $reservation->tour?->name ?? '—' }}</p>
    <p><strong>Client :</strong> {{ trim(($reservation->client_first_name ?? '').' '.($reservation->client_last_name ?? '')) }}</p>
    <p><strong>Statut :</strong> {{ $reservation->status }}</p>
    <p><strong>Total :</strong> {{ $reservation->total_price !== null ? number_format((float) $reservation->total_price, 2, ',', ' ').' DH' : '—' }}</p>
    <p><strong>Acompte (workspace) :</strong> {{ $reservation->paid_amount !== null ? number_format((float) $reservation->paid_amount, 2, ',', ' ').' DH' : '—' }}</p>
    <p>Généré le {{ $generatedAt->format('d/m/Y H:i') }}</p>

    <h2>Participants</h2>
    <table>
        <thead>
        <tr>
            <th>Nom</th>
            <th>Type</th>
            <th>Naissance</th>
            <th>Document</th>
        </tr>
        </thead>
        <tbody>
        @foreach($reservation->passengers as $p)
            <tr>
                <td>{{ trim(($p->first_name ?? '').' '.($p->last_name ?? '')) }}</td>
                <td>{{ $p->type }}</td>
                <td>{{ optional($p->birth_date)->format('d/m/Y') ?? '—' }}</td>
                <td>{{ $p->document_number ?? '—' }}</td>
            </tr>
        @endforeach
        </tbody>
    </table>

    @if($reservation->extras->isNotEmpty())
        <h2>Extras</h2>
        <table>
            <thead>
            <tr><th>Libellé</th><th>Prix</th></tr>
            </thead>
            <tbody>
            @foreach($reservation->extras as $e)
                <tr>
                    <td>{{ $e->name }}</td>
                    <td>{{ number_format((float) $e->price, 2, ',', ' ') }} DH</td>
                </tr>
            @endforeach
            </tbody>
        </table>
    @endif
</body>
</html>
