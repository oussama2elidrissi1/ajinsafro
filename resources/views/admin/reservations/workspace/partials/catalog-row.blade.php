...
<article class="catalog-card ws-catalog-row ws-offer-card ws-offer-card--compact admin-sales-catalogue-card-fix {{ $rowAccent }} {{ $hasLaravel ? '' : 'ws-catalog-row--unlinked' }}{{ $isNearFuture ? ' ws-catalog-row--near' : '' }}{{ $isSellable ? '' : ' ws-catalog-row--configure' }}"
    ...
    <div class="ws-offer-card__body ws-offer-card__body--compact">
        ...
        @elseif($viewMode === 'card')
            <div class="ws-offer-card__departures{{ $hasMultipleDepartures ? ' btn-ws-open-departures' : '' }}"
                @if($hasMultipleDepartures)
                    role="button" tabindex="0" data-row-code="{{ e($row['code']) }}" aria-label="Voir les départs disponibles"
                @endif>
                <div class="ws-offer-card__section-label">Prochain départ</div>
                @if($cardNextDepartureLabel)
                    @php
                ...
                @if($hasMultipleDepartures)
                    <button type="button" class="ws-offer-card__more btn-ws-open-departures" data-row-code="{{ e($row['code']) }}">
                        Voir tous les départs ({{ $departures->count() }})
                    </button>
                @endif
            </div>
