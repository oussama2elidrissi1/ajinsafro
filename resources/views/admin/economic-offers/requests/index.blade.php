...
@section('title', 'Demandes Formule Ã‰conomique')

@section('content')
    <x-admin.page-header
        title="Demandes Formule Ã‰conomique"
        subtitle="Suivez les demandes envoyÃ©es depuis la page publique."
        :breadcrumbs="[
    ...
    <x-admin.filter-panel :action="route('admin.economic-offers.requests.index')" :reset-url="route('admin.economic-offers.requests.index')" grid-class="row g-3 align-items-end">
        <x-slot name="fields">
            <div class="col-md-4">
                ...
                <input type="text" name="q" value="{{ $filters['q'] }}" class="form-control" placeholder="Nom, email, tÃ©lÃ©phone, offre">
            </div>
            <div class="col-md-3">
    ...
    <section class="aj-panel">
        @if($requests->isEmpty())
            <x-admin.empty-state title="Aucune demande" message="Les demandes du site public apparaÃ®tront ici." />
        @else
                ...
                <table class="aj-table" style="width:100%;">
                    ...
                    @foreach($requests as $requestItem)
                        <tr>
                            ...
                            <td>{{ $requestItem->offer_title ?: optional($requestItem->offer)->title ?: 'Non associÃ©e' }}</td>
                            ...
                            <td>{{ $requestItem->responsible_agent ?: 'â€”' }}</td>
                            <td class="text-end">
