<div class="ve-form-toolbar ve-pane-card">
    <div class="ve-form-actions-body">
        <div class="ve-form-toolbar__copy">
            <p class="ve-form-toolbar__eyebrow mb-1">Sauvegarde</p>
            <h5 class="mb-1">Enregistrer les modifications</h5>
            <p class="text-muted small mb-0">Les changements sont conserves sans quitter la page.</p>
        </div>

        <div class="ve-form-actions-buttons">
            <a href="{{ route('admin.circuits.voyages.index') }}" class="btn btn-outline-secondary">
                <i class="bx bx-arrow-back"></i> Retour a la liste
            </a>

            <button type="submit" form="edit-voyage-form" class="btn btn-primary" id="edit-voyage-submit-btn">
                <i class="bx bx-save"></i> Enregistrer
            </button>
        </div>
    </div>
</div>
