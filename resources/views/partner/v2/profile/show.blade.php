...
<div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
    <div class="xl:col-span-1 space-y-6">
        <div class="bg-white p-6 rounded-2xl shadow-custom border border-gray-100 text-center">
            <img src="{{ auth()->user()->avatar_url }}" alt="Avatar" class="w-28 h-28 rounded-full object-cover border-4 border-[#e6f3fa] shadow-sm mx-auto mb-4">
            <h3 class="font-bold text-[#0e3a5a] text-xl">{{ auth()->user()->name }}</h3>
            <p class="text-xs font-bold text-[#f37a1f] uppercase tracking-wider mt-1 mb-4">{{ $partner?->partner_type_label ?? 'Partenaire' }}</p>
            <div class="text-left space-y-2">
                <div class="flex items-center gap-3 text-sm text-gray-600 bg-gray-50 p-3 rounded-xl border border-gray-100">
                    <span class="font-medium">Email: {{ auth()->user()->email }}</span>
                </div>
                <div class="flex items-center gap-3 text-sm text-gray-600 bg-gray-50 p-3 rounded-xl border border-gray-100">
                    <span class="font-medium">Téléphone: {{ $partner?->telephone ?? auth()->user()->phone ?? '—' }}</span>
                </div>
                <div class="flex items-center gap-3 text-sm text-gray-600 bg-gray-50 p-3 rounded-xl border border-gray-100">
                    <span class="font-medium">Ville: {{ $partner?->ville ?? '—' }}</span>
                </div>
            </div>
        </div>
    </div>
