...
<div class="space-y-6 mt-8">
        ...
        <div class="bg-white rounded-2xl shadow-custom border border-gray-100 p-5">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div class="rounded-xl border border-gray-100 bg-[#e6f3fa]/30 p-3 text-center">
                    ...
                    <p class="text-2xl font-black text-[#0083c4]">{{ $statsPersonal['reservations_total'] ?? 0 }}</p>
                </div>
                <div class="rounded-xl border border-gray-100 p-3 text-center">
                    ...
                    <p class="text-2xl font-black text-[#0e3a5a]">{{ $statsPersonal['clients_count'] ?? 0 }}</p>
                </div>
                <div class="rounded-xl border border-gray-100 p-3 text-center">
                    <p class="text-[10px] text-gray-500 font-bold uppercase">En cours</p>
                    <p class="text-xl font-bold text-yellow-600">{{ $statsPersonal['reservations_en_cours'] ?? 0 }}</p>
                </div>
                <div class="rounded-xl border border-gray-100 p-3 text-center">
                    <p class="text-[10px] text-gray-500 font-bold uppercase">Validées</p>
                    <p class="text-xl font-bold text-green-600">{{ $statsPersonal['reservations_validees'] ?? 0 }}</p>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-2xl shadow-custom border border-amber-100 p-5 bg-amber-50/20">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                <div class="bg-white rounded-xl border border-gray-100 p-3 text-center">
                    ...
                    <p class="text-xl font-black text-[#0e3a5a]">{{ $statsTeamOnly['reservations_total'] ?? 0 }}</p>
                </div>
                <div class="bg-white rounded-xl border border-gray-100 p-3 text-center">
                    <p class="text-[10px] text-gray-400 font-bold uppercase">En cours</p>
                    <p class="text-xl font-black text-yellow-600">{{ $statsTeamOnly['reservations_en_cours'] ?? 0 }}</p>
                </div>
                <div class="bg-white rounded-xl border border-gray-100 p-3 text-center">
                    <p class="text-[10px] text-gray-400 font-bold uppercase">Validées</p>
                    <p class="text-xl font-black text-green-600">{{ $statsTeamOnly['reservations_validees'] ?? 0 }}</p>
                </div>
                <div class="bg-white rounded-xl border border-gray-100 p-3 text-center">
                    <p class="text-[10px] text-gray-400 font-bold uppercase">Clients équipe</p>
                    <p class="text-xl font-black text-[#f37a1f]">{{ $statsTeamOnly['clients_count'] ?? 0 }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-custom border border-gray-100 overflow-hidden">
        <div class="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
            ...
            <span class="text-xs text-gray-500 font-medium">{{ $directReports->count() }} agent(s)</span>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse min-w-[640px]">
                ...
                @forelse($teamAgentStats as $row)
                    @php $u = $row['user']; @endphp
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="py-3 px-6">
                            <span class="font-bold text-[#0e3a5a]">{{ $u->name }}</span>
                            @if($u->job_title)
                                <span class="block text-[10px] text-gray-500">{{ $u->job_title }}</span>
                            @endif
                        </td>
                        <td class="py-3 px-6 text-gray-600 text-xs">{{ $u->email ?: '—' }}</td>
                        <td class="py-3 px-6 text-center font-bold text-[#0e3a5a]">{{ $row['reservations_total'] }}</td>
                        <td class="py-3 px-6 text-center text-yellow-600 font-semibold">{{ $row['reservations_en_cours'] }}</td>
                        <td class="py-3 px-6 text-center text-green-600 font-semibold">{{ $row['reservations_validees'] }}</td>
                    </tr>
                @empty
                    <tr>
