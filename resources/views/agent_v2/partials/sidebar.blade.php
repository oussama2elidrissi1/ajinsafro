...
<aside class="w-full lg:w-72 shrink-0">
    <div class="bg-white rounded-2xl shadow-custom overflow-hidden sticky top-6 lg:top-8 border border-gray-100">
        ...
        <nav class="p-4 flex flex-col gap-1 max-h-[70vh] overflow-y-auto text-sm">
            ...
            <details class="agent-nav-group rounded-xl border border-transparent hover:border-gray-100 mt-1 {{ $profileActive ? 'bg-gray-50/80' : '' }}" {{ $profileActive ? 'open' : '' }}>
                ...
                <div class="pb-2 pt-0 flex flex-col gap-0.5 px-1">
                    @can('dashboard.view')
                        <a href="{{ route('admin.profile.edit') }}"
                           data-partner-nav
                           class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ $profileActive ? 'bg-[#e6f3fa]/60 text-[#0083c4] font-semibold' : 'text-gray-600 hover:bg-white hover:text-[#0083c4] font-medium' }} transition-colors">
                            <span class="w-1.5 h-1.5 rounded-full shrink-0 {{ $profileActive ? 'bg-[#0083c4]' : 'bg-gray-300' }}"></span>
