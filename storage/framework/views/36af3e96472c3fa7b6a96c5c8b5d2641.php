...
<div class="card mb-3 border" id="reservation-hotel-card">
    <div class="card-body">
        ...
        <?php if(! $legacyEdit): ?>
            ...
            <div class="row g-2 mb-3">
                <div class="col-md-6">
                    ...
                    <input type="hidden" name="departure_id" id="input-departure-id" value="<?php echo e($initialDepartureId); ?>">
                    <input type="hidden" name="travel_date_id" id="input-travel-date-id" value="<?php echo e($initialTravelDateId); ?>">
                </div>
                <div class="col-md-4">
                    <label class="form-label" for="reservation-base-price">Prix unitaire par voyageur (DH)</label>
                    <input type="number" id="reservation-base-price" name="base_price" class="form-control" value="<?php echo e(old('base_price', $reservation?->base_price ?? '')); ?>" min="0" step="0.01" placeholder="0.00" readonly>
                </div>
                <div class="col-md-8">
                    <label class="form-label" for="reservation-discount-value">Réduction</label>
                    <div class="input-group">
                        <input type="number" id="reservation-discount-value" name="discount_value" class="form-control" value="<?php echo e(old('discount_value', $reservation?->discount_value ?? '')); ?>" min="0" step="0.01" placeholder="0">
                        <select id="reservation-discount-type" name="discount_type" class="form-select" style="max-width: 96px;">
                            <option value="percentage" <?php if(old('discount_type', $reservation?->discount_type ?? 'percentage') === 'percentage'): echo 'selected'; endif; ?>>%</option>
                            <option value="fixed" <?php if(old('discount_type', $reservation?->discount_type ?? '') === 'fixed'): echo 'selected'; endif; ?>>DH</option>
                        </select>
                    </div>
        ...
        <div id="reservation-hotel-rooms-container">
            <?php if($legacyEdit): ?>
                <?php $roomFieldIndex = 0; ?>
                <?php $__currentLoopData = $tourHotelsWithRooms; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $hotel): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="card mb-2 reservation-hotel-block" data-tour-hotel-id="<?php echo e($hotel->id); ?>">
                        <div class="card-header bg-light py-2">
                            <strong><?php echo e($hotel->hotel_name ?: 'Hôtel J'.($hotel->check_in_day ?? '?')); ?></strong>
                            <?php if($hotel->check_in_day || $hotel->check_out_day): ?>
                                <span class="text-muted small">— J<?php echo e($hotel->check_in_day ?? '?'); ?> à J<?php echo e($hotel->check_out_day ?? '?'); ?></span>
                            <?php endif; ?>
                        </div>
                        <div class="card-body py-2">
                            <div class="table-responsive">
                                <table class="table table-sm table-bordered mb-0">
                                    ...
                                    <tbody>
                                        <?php $__currentLoopData = $hotel->rooms->where('is_active', true); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $room): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <?php
                                            ...
                                            <tr class="reservation-room-row">
                                                <td>
                                                    <?php echo e($room->room_type); ?><?php echo e($room->room_label ? ' — '.$room->room_label : ''); ?>

                                                </td>
                                                <td class="text-center"><?php echo e($room->capacity_total); ?> pers.</td>
                                                <td class="text-center"><?php echo e(number_format((float) $room->supplement, 0, ',', ' ')); ?> DH</td>
                                                <td class="text-center">
                                                    <input type="hidden" name="hotel_rooms[<?php echo e($idx); ?>][tour_hotel_id]" value="<?php echo e($hotel->id); ?>">
                                                    <input type="hidden" name="hotel_rooms[<?php echo e($idx); ?>][tour_hotel_room_id]" value="<?php echo e($room->id); ?>">
                                                    <input
                                                        type="number"
                                                        name="hotel_rooms[<?php echo e($idx); ?>][room_count]"
                                                        class="form-control form-control-sm text-center reservation-room-count"
                                                        value="<?php echo e($roomCount); ?>"
                                                        min="0"
                                                        data-room-supplement="<?php echo e($room->supplement); ?>"
                                                        data-room-capacity="<?php echo e($room->capacity_total); ?>"
                                                    >
                                                </td>
                                                <td class="text-end reservation-room-total"><?php echo e($roomCount > 0 ? number_format($roomCount * (float) $room->supplement, 0, ',', ' ').' DH' : '—'); ?></td>
                                            </tr>
        ...
        <?php if($roomsDebugEnabled && ! $compactAvailabilityOnly): ?>
        ...
        <div id="reservation-hotel-summary" class="mt-3 <?php echo e($legacyEdit && $tourHotelsWithRooms->isNotEmpty() ? '' : 'd-none'); ?>">
            <hr>
            <div class="row">
                <div class="col-md-4">
...
<script type="application/json" id="reservation-initial-departure-rooms"><?php echo json_encode($initialDepartureRoomCounts); ?></script>
...
(function () {
    ...
    var legacyEdit = <?php echo json_encode($legacyEdit, 15, 512) ?>;
    var compactAvailabilityOnly = <?php echo json_encode($compactAvailabilityOnly, 15, 512) ?>;
    var voyageDeparturesUrl = <?php echo json_encode($voyageDeparturesPath, 15, 512) ?>;
    var departureHotelsRoomsUrl = <?php echo json_encode($departureHotelsRoomsPath, 15, 512) ?>;
    var initialRooms = {};
    ...
    var roomsDebugEnabled = <?php echo json_encode($roomsDebugEnabled, 15, 512) ?>;
    ...
    if (tourSelect && !legacyEdit) {
        ...
        if (tourSelect.value) {
            loadDeparturesForTour(tourSelect.value, '<?php echo e($initialDepartureId); ?>');
        }
    } else {
        syncSummary();
    }
...
<?php /**PATH C:\Users\oussa\Desktop\themeforest-uMqxCtcU-qovex-laravel-admin-dashboard-template\Qovex_Laravel_v3.0.0\Admin\resources\views\admin\reservations\partials\_hotel_rooms.blade.php ENDPATH**/ ?>