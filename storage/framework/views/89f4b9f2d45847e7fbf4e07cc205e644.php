<div class="reservation-create__summary-card">
    <p class="reservation-create__eyebrow">R&eacute;sum&eacute; rapide</p>
    <div class="reservation-create__summary-item">
        <span>Prestation</span>
        <strong id="create-summary-trip">Aucune s&eacute;lection</strong>
    </div>
    <div class="reservation-create__summary-item">
        <span>D&eacute;part</span>
        <strong id="create-summary-departure">—</strong>
    </div>
    <div class="reservation-create__summary-item">
        <span>Voyageurs</span>
        <strong id="create-summary-travelers">1</strong>
    </div>
    <div class="reservation-create__summary-item">
        <span>Prix unitaire</span>
        <strong id="create-summary-unit-price">—</strong>
    </div>
    <div class="reservation-create__summary-item">
        <span>R&eacute;duction</span>
        <strong id="create-summary-discount">Aucune</strong>
    </div>
    <div class="reservation-create__summary-item">
        <span>Prix apr&egrave;s r&eacute;duction</span>
        <strong id="create-summary-price-after-discount">—</strong>
    </div>
    <div class="reservation-create__summary-item">
        <span>Total provisoire</span>
        <strong id="create-summary-total">—</strong>
    </div>
    <div class="reservation-create__summary-item">
        <span>Total pay&eacute;</span>
        <strong id="create-summary-paid">0 DH</strong>
    </div>
    <div class="reservation-create__summary-item">
        <span>Reste &agrave; payer</span>
        <strong id="create-summary-remaining">0 DH</strong>
    </div>
</div>
<?php /**PATH C:\Users\oussa\Desktop\themeforest-uMqxCtcU-qovex-laravel-admin-dashboard-template\Qovex_Laravel_v3.0.0\Admin\resources\views\admin\reservations\create\partials\summary.blade.php ENDPATH**/ ?>