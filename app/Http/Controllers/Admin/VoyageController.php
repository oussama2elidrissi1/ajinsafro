...
class VoyageController extends Controller
{
    ...
    public function locationsCities(Request $request): JsonResponse
    {
        ...
        $countryCodeByWpId = [];
        foreach ($countryCitiesData as $code => $country) {
            if (!empty($country['id'])) {
                $countryCodeByWpId[(int) $country['id']] = $code;
            }
        }

        $selectedCodes = [];
        foreach ($countryIds as $raw) {
            $value = is_string($raw) ? trim($raw) : (string) $raw;
            if ($value === '') {
                continue;
            }
            $upper = strtoupper($value);
            if (isset($worldCountries[$upper])) {
                $selectedCodes[$upper] = true;
                continue;
            }
            if (is_numeric($value)) {
                $wpCountryId = (int) $value;
                if (isset($countryCodeByWpId[$wpCountryId])) {
                    $selectedCodes[$countryCodeByWpId[$wpCountryId]] = true;
                }
            }
        }

        $selectedCodes = array_keys($selectedCodes);
        ...
        foreach ($selectedCodes as $code) {
            $countryName = $worldCountries[$code] ?? $code;
            $countryWpId = isset($countryCitiesData[$code]['id']) ? (int) $countryCitiesData[$code]['id'] : null;
            foreach (($mergedCitiesByCode[$code] ?? []) as $city) {
                $cityName = (string) ($city['title'] ?? '');
                if ($cityName === '') {
                    continue;
                }
                if ($queryLower !== '' && mb_stripos($cityName, $queryLower, 0, 'UTF-8') === false) {
                    continue;
                }
