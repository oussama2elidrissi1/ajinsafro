...
class WpTourRepository
{
    ...
    protected function updateTourMetas(WpPost $post, array $data): void
    {
        ...
        $metaMapping = [
            ...
            'commission_adulte_type' => 'commission_adulte_type',
            'commission_enfant' => 'commission_enfant',
            'commission_enfant_type' => 'commission_enfant_type',
            'discount' => 'discount',
        ...
        if (isset($data['featured_image'])) {
            ...
            if ($valid) {
                $this->media->setPostThumbnailIfValidWithPolicy($post, (int) $valid, [
                    'source' => 'WpTourRepository::updateTourMetas featured_image',
                ], true);
            }
        ...
        if (array_key_exists('thumbnail_id', $data)) {
            if ($data['thumbnail_id'] === '' || $data['thumbnail_id'] === null) {
                ...
                if ($valid) {
                    $this->media->setPostThumbnailIfValidWithPolicy($post, (int) $valid, [
                        'source' => 'WpTourRepository::updateTourMetas thumbnail_id',
                    ], true);
                }
                // Si invalide: ne rien changer (ne pas supprimer un thumbnail valide existant).
            }
