<?php

return [
    'items' => [
        [
            'key' => 'dashboard',
            'label' => 'Dashboard',
            'icon' => 'bx bx-home-circle',
            'permission' => 'dashboard.view',
            'children' => [
                ['label' => 'Vue d\'ensemble', 'route' => 'admin.dashboard.vue-globale', 'permission' => 'dashboard.overview.view'],
                ['label' => 'Statistiques', 'route' => 'admin.dashboard.statistiques', 'permission' => 'dashboard.stats.view'],
                ['label' => 'Alertes', 'route' => 'admin.dashboard.alertes', 'permission' => 'dashboard.alerts.view'],
            ],
        ],
        [
            'key' => 'reservations',
            'label' => 'Réservations',
            'icon' => 'bx bx-calendar-check',
            'permission' => 'reservations.view',
            'active_patterns' => ['admin.vente.catalogue', 'admin.reservations.*', 'admin.custom-requests.*'],
            'children' => [
                ['label' => 'Catalogue de produits', 'route' => 'admin.vente.catalogue', 'permission' => 'reservations.view', 'active_patterns' => ['admin.vente.catalogue', 'admin.reservations.workspace']],
                ['label' => 'Réservations agents', 'route' => 'admin.reservation-dossiers.index', 'permission' => 'reservations.view', 'active_patterns' => ['admin.reservation-dossiers.*', 'admin.reservations.index']],
                ['label' => 'Réservations partenaires', 'route' => 'admin.reservations.partners', 'permission' => 'reservations.view', 'active_patterns' => ['admin.reservations.partners']],
                ['label' => 'Réservations en ligne', 'route' => 'admin.reservations.clients', 'permission' => 'reservations.view', 'active_patterns' => ['admin.reservations.clients']],
                [
                    'label' => 'Demande à la carte',
                    'icon' => 'bx bx-edit-alt',
                    'permission' => ['reservations.view', 'custom_requests.view'],
                    'active_patterns' => ['admin.reservations.custom-requests.*', 'admin.tailor-made-requests.*', 'admin.custom-requests.*'],
                    'children' => [
                        ['label' => 'Demandes à la carte', 'route' => 'admin.custom-requests.index', 'permission' => 'custom_requests.view', 'active_patterns' => ['admin.custom-requests.index', 'admin.custom-requests.show']],
                        ['label' => 'Nouvelle demande à la carte', 'route' => 'admin.custom-requests.create', 'permission' => 'custom_requests.create', 'emails' => ['dev@ajinsafro.ma', 'dev-hiba@ajinsafro.ma'], 'active_patterns' => ['admin.custom-requests.create', 'admin.custom-requests.store']],
                        ['label' => 'Demandes à traiter', 'route' => 'admin.custom-requests.index', 'query' => ['status' => 'new'], 'permission' => 'custom_requests.quote'],
                        ['label' => 'Mes cotations', 'route' => 'admin.custom-requests.index', 'query' => ['assigned_to' => 'me'], 'permission' => 'custom_requests.quote'],
                        ['label' => 'Demandes urgentes', 'route' => 'admin.custom-requests.index', 'query' => ['priority' => 'urgent'], 'permission' => 'custom_requests.quote'],
                        ['label' => 'Toutes les demandes à la carte', 'route' => 'admin.custom-requests.index', 'permission' => 'custom_requests.view_all'],
                        ['label' => 'Demandes à la carte', 'route' => 'admin.reservations.custom-requests.index', 'permission' => 'reservations.view', 'active_patterns' => ['admin.reservations.custom-requests.*']],
                        ['label' => 'Demandes à la carte en ligne', 'route' => 'admin.tailor-made-requests.index', 'permission' => 'reservations.view', 'active_patterns' => ['admin.tailor-made-requests.*']],
                    ],
                ],
            ],
        ],
        [
            'key' => 'messagerie',
            'label' => 'Messagerie',
            'icon' => 'bx bx-envelope',
            'permission' => 'messagerie.view',
            'route' => 'admin.messagerie.index',
            'active_patterns' => ['admin.messagerie.*'],
        ],
        [
            'key' => 'dev_reclamations',
            'label' => 'Reclamations dev',
            'icon' => 'bx bx-message-square-error',
            'route' => 'admin.dev.reclamations.index',
            'emails' => ['dev@ajinsafro.ma', 'dev-hiba@ajinsafro.ma'],
            'active_patterns' => ['admin.dev.reclamations.*'],
        ],
        [
            'key' => 'customers',
            'label' => 'Clients',
            'icon' => 'bx bx-user',
            'permission' => 'customers.view',
            'children' => [
                ['label' => 'Liste clients', 'route' => 'admin.customers.clients.index', 'permission' => 'customers.clients.view', 'active_patterns' => ['admin.customers.clients.*']],
                ['label' => 'Prospects', 'route' => 'admin.customers.prospects', 'permission' => 'customers.clients.view'],
                ['label' => 'Voyageurs', 'route' => 'admin.customers.voyageurs', 'permission' => 'customers.travelers.view'],
                ['label' => 'Historique', 'route' => 'admin.customers.historique', 'permission' => 'customers.history.view'],
                ['label' => 'Fidélité', 'route' => 'admin.customers.fidelite', 'permission' => 'customers.loyalty.view'],
            ],
        ],
        [
            'key' => 'agencies',
            'label' => 'Points de vente',
            'icon' => 'bx bx-buildings',
            'permission' => ['points_of_sale.view', 'agencies.view'],
            'children' => [
                ['label' => 'Liste des points de vente', 'route' => 'admin.points-of-sale.index', 'permission' => ['points_of_sale.view', 'agencies.view'], 'active_patterns' => ['admin.points-of-sale.index', 'admin.points-of-sale.show', 'admin.agencies.index', 'admin.agencies.show']],
                ['label' => 'Créer une agence', 'route' => 'admin.agencies.create', 'permission' => 'agencies.create', 'active_patterns' => ['admin.agencies.create', 'admin.agencies.store']],
                ['label' => 'Performance agences', 'route' => 'admin.agencies.performance', 'permission' => 'agency_performance.view', 'active_patterns' => ['admin.agencies.performance']],
                ['label' => 'Modifier les agences', 'route' => 'admin.agencies.index', 'permission' => 'agencies.edit'],
                ['label' => 'Archiver les agences', 'route' => 'admin.agencies.index', 'permission' => 'agencies.delete'],
                ['label' => 'Employés des agences', 'route' => 'admin.agency-employees.index', 'permission' => 'agency_employees.view', 'active_patterns' => ['admin.agency-employees.*']],
                ['label' => 'Comptes agences', 'route' => 'admin.agency-accounts.index', 'permission' => 'agency_accounts.view', 'active_patterns' => ['admin.agency-accounts.*']],
                ['label' => 'Affectations', 'route' => 'admin.assignments.index', 'permission' => 'assignments.view', 'active_patterns' => ['admin.assignments.*']],
                ['label' => 'Créer les employés des agences', 'route' => 'admin.agency-employees.index', 'permission' => 'agency_employees.create'],
                ['label' => 'Modifier les employés des agences', 'route' => 'admin.agency-employees.index', 'permission' => 'agency_employees.edit'],
                ['label' => 'Supprimer les employés des agences', 'route' => 'admin.agency-employees.index', 'permission' => 'agency_employees.delete'],
                ['label' => 'Commissions agences', 'route' => 'admin.agencies.performance', 'query' => ['tab' => 'commissions'], 'permission' => 'agency_commissions.view'],
            ],
        ],
        [
            'key' => 'products',
            'label' => 'Grille commerciale',
            'icon' => 'bx bx-store',
            'permission' => 'products.view',
            'children' => [
                ['label' => 'Tarifs', 'route' => 'admin.products.tarifs', 'permission' => 'products.pricing.view'],
                ['label' => 'Services', 'route' => 'admin.products.services', 'permission' => 'products.services.view'],
                ['label' => 'Conditions', 'route' => 'admin.products.conditions', 'permission' => 'products.conditions.view'],
                ['label' => 'Options', 'route' => 'admin.products.options', 'permission' => 'products.options.view'],
            ],
        ],
        [
            'key' => 'products_services',
            'label' => 'Produits & Services',
            'icon' => 'bx bx-layer',
            'permission' => 'products-services.view',
            'children' => [
                [
                    'key' => 'billetterie',
                    'label' => 'Billetterie',
                    'icon' => 'bx bx-ticket',
                    'permission' => 'products-services.view',
                    'children' => [
                        ['label' => 'Espace billetterie', 'route' => 'admin.menu-hubs.billetterie', 'permission' => 'products-services.view', 'active_patterns' => ['admin.menu-hubs.billetterie']],
                    ],
                ],
                [
                    'key' => 'voyages',
                    'label' => 'Voyages',
                    'icon' => 'bx bx-plane',
                    'permission' => 'circuits.voyages.view',
                    'active_patterns' => ['admin.circuits.voyages.*'],
                    'children' => [
                        ['label' => 'Tous les voyages', 'route' => 'admin.circuits.voyages.index', 'permission' => 'circuits.voyages.view'],
                        ['label' => 'Départs / disponibilités', 'route' => 'admin.circuits.departs-dates', 'permission' => 'circuits.departures.view'],
                        ['label' => 'Hôtels circuit', 'route' => 'admin.circuits.tour-hotels.index', 'permission' => 'circuits.tour-hotels.view'],
                        ['label' => 'Chambres', 'route' => 'admin.accommodations.chambres', 'permission' => 'accommodations.rooms.view'],
                        ['label' => 'Tarifs / disponibilités', 'route' => 'admin.accommodations.tarifs-saisonniers', 'permission' => 'accommodations.seasonal-pricing.view'],
                    ],
                ],
                [
                    'key' => 'group_deals',
                    'label' => 'Group Deals',
                    'icon' => 'bx bx-group',
                    'permission' => 'group-deals.view',
                    'children' => [
                        ['label' => 'Catalogue Group Deals', 'route' => 'admin.group-deals.index', 'permission' => 'group-deals.offers.view', 'active_patterns' => ['admin.group-deals.index', 'admin.group-deals.create', 'admin.group-deals.store', 'admin.group-deals.show', 'admin.group-deals.edit', 'admin.group-deals.update', 'admin.group-deals.destroy', 'admin.group-deals.recalculate']],
                        ['label' => 'Voyages GD', 'route' => 'admin.group-deals.trips.index', 'permission' => 'group-deals.trips.view', 'active_patterns' => ['admin.group-deals.trips.*']],
                        ['label' => 'Départs GD', 'route' => 'admin.group-deals.departures.index', 'permission' => 'group-deals.departures.view', 'active_patterns' => ['admin.group-deals.departures.index', 'admin.group-deals.departures.show', 'admin.group-deals.departures.recalculate']],
                        ['label' => 'Participants / inscriptions', 'route' => 'admin.group-deals.participants.index', 'permission' => 'group-deals.participants.view', 'active_patterns' => ['admin.group-deals.participants.*', 'admin.group-deals.departures.participants.*']],
                        ['label' => 'Tarifs par palier', 'route' => 'admin.group-deals.tiers.index', 'permission' => 'group-deals.tiers.view', 'active_patterns' => ['admin.group-deals.tiers.*', 'admin.group-deals.trips.tiers.*']],
                    ],
                ],
                [
                    'key' => 'accommodations',
                    'label' => 'Hébergements',
                    'icon' => 'bx bx-hotel',
                    'permission' => 'accommodations.view',
                    'children' => [
                        ['label' => 'Espace hébergement', 'route' => 'admin.menu-hubs.hebergement', 'permission' => ['accommodations.view', 'products-services.view'], 'active_patterns' => ['admin.menu-hubs.hebergement']],
                        ['label' => 'Tous les hébergements', 'route' => 'admin.accommodations.hotels', 'permission' => 'accommodations.catalog.view'],
                        ['label' => 'Hôtels WordPress', 'route' => 'admin.wordpress.hotels.index', 'permission' => 'accommodations.wordpress-hotels.view'],
                        ['label' => 'Packs hébergement', 'route' => 'admin.accommodation-packages.index', 'permission' => 'accommodations.packages.view'],
                        ['label' => 'Chambres', 'route' => 'admin.accommodations.chambres', 'permission' => 'accommodations.rooms.view'],
                        ['label' => 'Tarifs saisonniers', 'route' => 'admin.accommodations.tarifs-saisonniers', 'permission' => 'accommodations.seasonal-pricing.view'],
                        ['label' => 'Disponibilités', 'route' => 'admin.accommodations.disponibilites', 'permission' => 'accommodations.availability.view'],
                    ],
                ],
                [
                    'key' => 'hajj_omra',
                    'label' => 'Hajj & Omra',
                    'icon' => 'bx bx-building-house',
                    'permission' => 'hajj-omra.view',
                    'children' => [
                        ['label' => 'Espace Hajj & Omra', 'route' => 'admin.menu-hubs.hajj-omra', 'permission' => 'hajj-omra.view', 'active_patterns' => ['admin.menu-hubs.hajj-omra']],
                        ['label' => 'Toutes les offres', 'route' => 'admin.hajj-omra.index', 'permission' => 'hajj-omra.view', 'active_patterns' => ['admin.hajj-omra.index', 'admin.hajj-omra.create', 'admin.hajj-omra.store', 'admin.hajj-omra.show', 'admin.hajj-omra.edit', 'admin.hajj-omra.update', 'admin.hajj-omra.destroy']],
                        ['label' => 'Demandes', 'route' => 'admin.hajj-omra.requests.index', 'permission' => 'hajj-omra.requests.view', 'active_patterns' => ['admin.hajj-omra.requests.*']],
                    ],
                ],
                [
                    'key' => 'economic_offers',
                    'label' => 'Formule économique',
                    'icon' => 'bx bx-wallet-alt',
                    'permission' => 'economic-offers.view',
                    'children' => [
                        ['label' => 'Espace formule low cost', 'route' => 'admin.menu-hubs.low-cost', 'permission' => 'economic-offers.view', 'active_patterns' => ['admin.menu-hubs.low-cost']],
                        ['label' => 'Toutes les offres', 'route' => 'admin.economic-offers.index', 'permission' => 'economic-offers.view', 'active_patterns' => ['admin.economic-offers.index', 'admin.economic-offers.create', 'admin.economic-offers.store', 'admin.economic-offers.show', 'admin.economic-offers.edit', 'admin.economic-offers.update', 'admin.economic-offers.destroy']],
                        ['label' => 'Demandes', 'route' => 'admin.economic-offers.requests.index', 'permission' => 'economic-offers.requests.view', 'active_patterns' => ['admin.economic-offers.requests.*']],
                    ],
                ],
                [
                    'key' => 'activities',
                    'label' => 'Activités',
                    'icon' => 'bx bx-camera',
                    'permission' => 'activities.view',
                    'children' => [
                        ['label' => 'Espace activités', 'route' => 'admin.menu-hubs.activites', 'permission' => 'activities.view', 'active_patterns' => ['admin.menu-hubs.activites']],
                        ['label' => 'Offres activités', 'route' => 'admin.activity-offers.index', 'permission' => 'activities.offers.view'],
                        ['label' => 'Catégories', 'route' => 'admin.activities.categories', 'permission' => 'activities.categories.view'],
                        ['label' => 'Images / galerie', 'route' => 'admin.activities.gallery', 'permission' => 'activities.gallery.view'],
                        ['label' => 'Disponibilités', 'route' => 'admin.activities.availability', 'permission' => 'activities.availability.view'],
                    ],
                ],
                [
                    'key' => 'transfers',
                    'label' => 'Transferts',
                    'icon' => 'bx bx-car',
                    'permission' => 'transfers.view',
                    'children' => [
                        ['label' => 'Espace transferts', 'route' => 'admin.menu-hubs.transfers', 'permission' => 'transfers.view', 'active_patterns' => ['admin.menu-hubs.transfers']],
                        ['label' => 'Offres transfert', 'route' => 'admin.circuits.tour-transfers.index', 'permission' => 'transfers.offers.view'],
                        ['label' => 'Véhicules', 'route' => 'admin.transfers.vehicles', 'permission' => 'transfers.vehicles.view'],
                        ['label' => 'Tarifs', 'route' => 'admin.transfers.pricing', 'permission' => 'transfers.pricing.view'],
                        ['label' => 'Disponibilités', 'route' => 'admin.transfers.availability', 'permission' => 'transfers.availability.view'],
                    ],
                ],
            ],
        ],
        [
            'key' => 'operations',
            'label' => 'Opérations Terrain',
            'icon' => 'bx bx-briefcase-alt-2',
            'permission' => 'operations.view',
            'children' => [
                ['label' => 'Planning', 'route' => 'admin.operations.planning', 'permission' => 'operations.planning.view'],
                ['label' => 'Véhicules', 'route' => 'admin.operations.vehicules', 'permission' => 'operations.vehicles.view'],
                ['label' => 'Guides & Chauffeurs', 'route' => 'admin.operations.guides-chauffeurs', 'permission' => 'operations.guides.view'],
                ['label' => 'Logistique', 'route' => 'admin.operations.logistique', 'permission' => 'operations.logistics.view'],
            ],
        ],
        [
            'key' => 'visa',
            'label' => 'Visa',
            'icon' => 'bx bx-id-card',
            'permission' => 'visa.view',
            'children' => [
                ['label' => 'Espace visa', 'route' => 'admin.menu-hubs.visa', 'permission' => 'visa.view', 'active_patterns' => ['admin.menu-hubs.visa']],
                ['label' => 'Demandes de visa', 'route' => 'admin.visa.demandes-visa', 'permission' => 'visa.requests.view'],
                ['label' => 'Documents', 'route' => 'admin.visa.documents', 'permission' => 'visa.documents.view'],
                ['label' => 'Statuts', 'route' => 'admin.visa.statuts', 'permission' => 'visa.statuses.view'],
            ],
        ],
        [
            'key' => 'finance',
            'label' => 'Finance',
            'icon' => 'bx bx-wallet',
            'permission' => ['finance.view', 'departures_finance.view', 'commissions.view-own', 'commissions.view-team', 'commissions.view-all'],
            'children' => [
                ['label' => 'Mes commissions', 'route' => 'admin.agent.commissions.index', 'permission' => ['commissions.view-own', 'commissions.view-team', 'commissions.view-all'], 'active_patterns' => ['admin.agent.commissions.*']],
                ['label' => 'Factures', 'route' => 'admin.finance.factures', 'permission' => 'finance.invoices.view'],
                ['label' => 'Paiements', 'route' => 'admin.finance.paiements', 'permission' => 'finance.payments.view'],
                ['label' => 'Dépenses', 'route' => 'admin.finance.depenses', 'permission' => 'finance.expenses.view'],
                ['label' => 'Finances departs', 'route' => 'admin.finance.departures.index', 'permission' => 'departures_finance.view', 'active_patterns' => ['admin.finance.departures.*']],
                ['label' => 'Commissions', 'route' => 'admin.finance.commissions', 'permission' => 'commissions.view-all', 'active_patterns' => ['admin.finance.commissions*']],
                ['label' => 'Rapports financiers', 'route' => 'admin.finance.rapports-financiers', 'permission' => 'finance.reports.view'],
            ],
        ],
        [
            // Module reserve a l'administration : la cle `gate` applique exactement la meme
            // regle que le middleware `finance.control`, et ne peut pas etre accordee
            // depuis l'ecran Roles & Permissions.
            'key' => 'finance-control',
            'label' => 'Finance & Contrôle',
            'icon' => 'bx bx-line-chart',
            'gate' => 'finance-control.access',
            'permission' => 'finance.projects.view',
            'children' => [
                ['label' => 'Vue d\'ensemble', 'route' => 'admin.finance.control.dashboard', 'gate' => 'finance-control.access', 'permission' => 'finance.projects.view'],
                ['label' => 'Projets de voyage', 'route' => 'admin.finance.control.travel-projects.index', 'gate' => 'finance-control.access', 'permission' => 'finance.projects.view', 'active_patterns' => ['admin.finance.control.travel-projects.*']],
                ['label' => 'Encaissements clients', 'route' => 'admin.finance.control.client-collections.index', 'gate' => 'finance-control.access', 'permission' => 'finance.projects.view'],
                ['label' => 'Charges voyages', 'route' => 'admin.finance.control.travel-expenses.index', 'gate' => 'finance-control.access', 'permission' => 'finance.expenses.manage', 'active_patterns' => ['admin.finance.control.travel-expenses.*']],
                ['label' => 'Charges de structure', 'route' => 'admin.finance.control.structural-expenses.index', 'gate' => 'finance-control.access', 'permission' => 'finance.structural_expenses.manage', 'active_patterns' => ['admin.finance.control.structural-expenses.*']],
                ['label' => 'Fournisseurs', 'route' => 'admin.finance.control.suppliers.index', 'gate' => 'finance-control.access', 'permission' => 'finance.expenses.manage'],
                ['label' => 'Trésorerie', 'route' => 'admin.finance.control.treasury.index', 'gate' => 'finance-control.access', 'permission' => 'finance.treasury.view'],
                ['label' => 'Justificatifs', 'route' => 'admin.finance.control.documents.index', 'gate' => 'finance-control.access', 'permission' => 'finance.documents.manage', 'active_patterns' => ['admin.finance.control.documents.*']],
                ['label' => 'Résultats & marges', 'route' => 'admin.finance.control.results.index', 'gate' => 'finance-control.access', 'permission' => 'finance.reporting.view'],
                ['label' => 'Exports comptables', 'route' => 'admin.finance.control.exports.index', 'gate' => 'finance-control.access', 'permission' => 'finance.reporting.view', 'active_patterns' => ['admin.finance.control.exports.*']],
            ],
        ],
        [
            'key' => 'partners',
            'label' => 'Partenaires',
            'icon' => 'bx bx-group',
            'permission' => 'partners.view',
            'children' => [
                ['label' => 'Partenaires', 'route' => 'admin.partners.partenaires', 'permission' => 'partners.list.view'],
                ['label' => 'Demandes wallet', 'route' => 'admin.partners.wallet-requests', 'permission' => 'partners.view'],
                ['label' => 'Fournisseurs', 'route' => 'admin.partners.fournisseurs', 'permission' => 'partners.suppliers.view'],
                ['label' => 'Contrats', 'route' => 'admin.partners.contrats', 'permission' => 'partners.contracts.view'],
            ],
        ],
        [
            'key' => 'reporting',
            'label' => 'Reporting',
            'icon' => 'bx bx-bar-chart-alt-2',
            'permission' => 'reporting.view',
            'children' => [
                ['label' => 'Rapports', 'route' => 'admin.reporting.rapports', 'permission' => 'reporting.reports.view'],
                ['label' => 'Tableaux de bord', 'route' => 'admin.reporting.tableaux-bord', 'permission' => 'reporting.dashboards.view'],
                ['label' => 'Exports', 'route' => 'admin.reporting.exports', 'permission' => 'reporting.exports.view'],
            ],
        ],
        [
            'key' => 'settings',
            'label' => 'Paramètres',
            'icon' => 'bx bx-cog',
            'permission' => 'settings.view',
            'children' => [
                ['label' => 'Gestion RH', 'route' => 'admin.menu-hubs.rh', 'permission' => 'settings.users.manage', 'active_patterns' => ['admin.menu-hubs.rh']],
                ['label' => 'Utilisateurs', 'route' => 'admin.settings.utilisateurs', 'permission' => 'settings.users.manage'],
                ['label' => 'Rôles & Permissions', 'route' => 'admin.settings.roles-permissions', 'permission' => 'settings.roles.manage'],
                ['label' => 'Paramètres généraux', 'route' => 'admin.settings.parametres-generaux', 'permission' => 'settings.general.manage'],
                ['label' => 'Référentiels métier', 'route' => 'admin.settings.referentiels-metier', 'permission' => 'settings.general.manage'],
                ['label' => 'Types de charges', 'route' => 'admin.settings.charge-types.index', 'permission' => 'charge_types.manage', 'active_patterns' => ['admin.settings.charge-types.*']],
                ['label' => 'Home page', 'route' => 'admin.settings.home-page.edit', 'permission' => 'settings.general.manage'],
                ['label' => 'Bannière Voyages', 'route' => 'admin.settings.page-banners.voyages.edit', 'permission' => 'settings.general.manage'],
                ['label' => 'Sécurité', 'route' => 'admin.settings.securite', 'permission' => 'settings.security.manage'],
            ],
        ],
    ],

    'route_permissions' => [
        'admin.dashboard' => 'dashboard.view',
        'admin.dashboard.v4' => 'dashboard.view',
        'admin.dashboard.v3' => 'dashboard.view',
        'admin.dashboard.v5' => 'dashboard.view',
        'admin.profile.edit' => ['dashboard.view', 'reservations.view'],
        'admin.profile.update' => ['dashboard.view', 'reservations.view'],
        'admin.profile.avatar' => ['dashboard.view', 'reservations.view'],
        'admin.reservation-dossiers.index' => 'reservations.view',
        'admin.reservation-dossiers.show' => 'reservations.view',
        'admin.vente.catalogue' => 'reservations.view',
        'admin.reservations.workspace' => 'reservations.view',
        'admin.reservations.workspace.store' => 'reservations.view',
        'admin.reservations.workspace.prestation.participants' => 'reservations.view',
        'admin.reservations.workspace.prestation.pdf' => 'reservations.view',
        'admin.reservations.workspace.reservation.pdf' => 'reservations.view',

        // Reservation create flow (client lookup/creation) should be usable for sales roles
        // without granting full access to the Customers module.
        'admin.customers.clients.search' => ['customers.view', 'reservations.view'],
        'admin.customers.clients.quick-store' => ['customers.view', 'reservations.view'],

        'admin.menu-hubs.billetterie' => 'products-services.view',
        'admin.menu-hubs.hebergement' => 'accommodations.view',
        'admin.menu-hubs.hajj-omra' => 'hajj-omra.view',
        'admin.menu-hubs.low-cost' => 'economic-offers.view',
        'admin.menu-hubs.activites' => 'activities.view',
        'admin.menu-hubs.transfers' => 'transfers.view',
        'admin.menu-hubs.visa' => 'visa.view',
        'admin.menu-hubs.rh' => 'settings.users.manage',

        'admin.circuits.index' => 'circuits.voyages.view',
        'admin.circuits.circuits' => 'circuits.voyages.view',
        'admin.circuits.itineraires' => 'circuits.voyages.view',
        'admin.circuits.options' => 'circuits.voyages.view',
        'admin.circuits.politiques-conditions' => 'circuits.voyages.view',
        'admin.circuits.departs-dates' => 'circuits.departures.view',

        'admin.accommodations.index' => 'accommodations.view',
        'admin.accommodations.hotels' => 'accommodations.catalog.view',
        'admin.accommodations.chambres' => 'accommodations.rooms.view',
        'admin.accommodations.tarifs-saisonniers' => 'accommodations.seasonal-pricing.view',
        'admin.accommodations.disponibilites' => 'accommodations.availability.view',

        'admin.wordpress.hotels.index' => 'accommodations.wordpress-hotels.view',
        'admin.wordpress.hotels.create' => 'accommodations.wordpress-hotels.view',
        'admin.wordpress.hotels.store' => 'accommodations.wordpress-hotels.view',
        'admin.wordpress.hotels.edit' => 'accommodations.wordpress-hotels.view',
        'admin.wordpress.hotels.update' => 'accommodations.wordpress-hotels.view',
        'admin.wordpress.hotels.destroy' => 'accommodations.wordpress-hotels.view',

        'admin.activities.categories' => 'activities.categories.view',
        'admin.activities.gallery' => 'activities.gallery.view',
        'admin.activities.availability' => 'activities.availability.view',
        'admin.transfers.vehicles' => 'transfers.vehicles.view',
        'admin.transfers.pricing' => 'transfers.pricing.view',
        'admin.transfers.availability' => 'transfers.availability.view',

        'admin.group-deals.participants.index' => 'group-deals.participants.view',
        'admin.group-deals.participants.store' => 'group-deals.participants.view',
        'admin.group-deals.participants.update' => 'group-deals.participants.view',
        'admin.hajj-omra.requests.index' => 'hajj-omra.requests.view',
        'admin.hajj-omra.requests.show' => 'hajj-omra.requests.view',
        'admin.hajj-omra.requests.update' => 'hajj-omra.requests.view',
        'admin.economic-offers.requests.index' => 'economic-offers.requests.view',
        'admin.economic-offers.requests.show' => 'economic-offers.requests.view',
        'admin.economic-offers.requests.update' => 'economic-offers.requests.view',
        'admin.group-deals.tiers.index' => 'group-deals.tiers.view',
        'admin.group-deals.tiers.store' => 'group-deals.tiers.view',
        'admin.group-deals.tiers.update' => 'group-deals.tiers.view',
        'admin.group-deals.trips.tiers.store' => 'group-deals.tiers.view',
        'admin.group-deals.trips.tiers.update' => 'group-deals.tiers.view',
        'admin.group-deals.trips.tiers.destroy' => 'group-deals.tiers.view',
        'admin.group-deals.departures.participants.store' => 'group-deals.participants.view',
        'admin.group-deals.departures.participants.update' => 'group-deals.participants.view',
        'admin.group-deals.departures.recalculate' => 'group-deals.departures.view',

        'admin.settings.index' => 'settings.view',
        'admin.settings.parametres-generaux.update' => 'settings.general.manage',
        'admin.settings.referentiels-metier' => 'settings.general.manage',
        'admin.settings.referentiels-metier.group' => 'settings.general.manage',
        'admin.settings.referentiels-metier.store' => 'settings.general.manage',
        'admin.settings.referentiels-metier.update' => 'settings.general.manage',
        'admin.settings.referentiels-metier.destroy' => 'settings.general.manage',
        'admin.settings.referentiels-metier.import-legacy' => 'settings.general.manage',
        'admin.settings.home-page.edit' => 'settings.general.manage',
        'admin.settings.home-page.update' => 'settings.general.manage',
        'admin.settings.page-banners.voyages.edit' => 'settings.general.manage',
        'admin.settings.page-banners.voyages.update' => 'settings.general.manage',
        'admin.settings.page-banners.voyages.destroy-image' => 'settings.general.manage',
        'admin.settings.utilisateurs.create' => 'settings.users.manage',
        'admin.settings.utilisateurs' => 'settings.users.manage',
        'admin.settings.utilisateurs.store' => 'settings.users.manage',
        'admin.settings.utilisateurs.edit' => 'settings.users.manage',
        'admin.settings.utilisateurs.update' => 'settings.users.manage',
        'admin.settings.utilisateurs.toggle-active' => 'settings.users.manage',
        'admin.settings.utilisateurs.destroy' => 'settings.users.manage',
        'admin.settings.roles-permissions' => 'settings.roles.manage',
        'admin.settings.roles-permissions.create' => 'settings.roles.manage',
        'admin.settings.roles-permissions.store' => 'settings.roles.manage',
        'admin.settings.roles-permissions.edit' => 'settings.roles.manage',
        'admin.settings.roles-permissions.update' => 'settings.roles.manage',
        'admin.settings.roles-permissions.destroy' => 'settings.roles.manage',
        'admin.agencies.index' => ['agencies.view', 'points_of_sale.view'],
        'admin.agencies.show' => ['agencies.view', 'points_of_sale.view'],
        'admin.agencies.create' => ['agencies.create', 'points_of_sale.create'],
        'admin.agencies.store' => ['agencies.create', 'points_of_sale.create'],
        'admin.agencies.edit' => ['agencies.edit', 'points_of_sale.edit'],
        'admin.agencies.update' => ['agencies.edit', 'points_of_sale.edit'],
        'admin.agencies.toggle-status' => ['agencies.edit', 'points_of_sale.edit'],
        'admin.agencies.destroy' => ['agencies.delete', 'points_of_sale.delete'],
        'admin.agencies.performance' => ['agency_performance.view', 'points_of_sale.performance'],
        'admin.agencies.dashboard' => 'agency_dashboard.view',
        'admin.points-of-sale.index' => ['points_of_sale.view', 'agencies.view'],
        'admin.points-of-sale.show' => ['points_of_sale.view', 'agencies.view'],
        'admin.points-of-sale.create' => ['points_of_sale.create', 'agencies.create'],
        'admin.points-of-sale.store' => ['points_of_sale.create', 'agencies.create'],
        'admin.points-of-sale.edit' => ['points_of_sale.edit', 'agencies.edit'],
        'admin.points-of-sale.update' => ['points_of_sale.edit', 'agencies.edit'],
        'admin.points-of-sale.destroy' => ['points_of_sale.delete', 'agencies.delete'],
        'admin.points-of-sale.performance' => ['points_of_sale.performance', 'agency_performance.view'],
        'admin.agency-accounts.index' => 'agency_accounts.view',
        'admin.agency-accounts.show' => 'agency_accounts.view',
        'admin.agency-accounts.create' => 'agency_accounts.create',
        'admin.agency-accounts.store' => 'agency_accounts.create',
        'admin.agency-accounts.edit' => 'agency_accounts.edit',
        'admin.agency-accounts.update' => 'agency_accounts.edit',
        'admin.agency-accounts.disable' => 'agency_accounts.disable',
        'admin.agency-accounts.reset-password' => 'agency_accounts.reset_password',
        'admin.assignments.index' => 'assignments.view',
        'admin.assignments.store' => 'assignments.create',
        'admin.assignments.update' => 'assignments.edit',
        'admin.assignments.bulk' => 'assignments.bulk',
        'admin.assignments.remove' => 'assignments.remove',
        'admin.agency-employees.index' => ['agency_employees.view', 'pos_employees.view'],
        'admin.agency-employees.show' => ['agency_employees.view', 'pos_employees.view'],
        'admin.agency-employees.create' => ['agency_employees.create', 'pos_employees.create'],
        'admin.agency-employees.store' => ['agency_employees.create', 'pos_employees.create'],
        'admin.agency-employees.edit' => ['agency_employees.edit', 'pos_employees.edit'],
        'admin.agency-employees.update' => ['agency_employees.edit', 'pos_employees.edit'],
        'admin.agency-employees.destroy' => ['agency_employees.delete', 'pos_employees.delete'],
        'admin.agent.commissions.index' => ['commissions.view-own', 'commissions.view-team', 'commissions.view-all'],
        'admin.agent.commissions.show' => ['commissions.view-own', 'commissions.view-team', 'commissions.view-all'],
        'admin.tailor-made-requests.index' => 'reservations.view',
        'admin.tailor-made-requests.show' => 'reservations.view',
        'admin.tailor-made-requests.status' => 'reservations.update',
        'admin.tailor-made-requests.destroy' => 'reservations.delete',
        'admin.custom-requests.index' => 'custom_requests.view',
        'admin.custom-requests.create' => 'custom_requests.create',
        'admin.custom-requests.store' => 'custom_requests.create',
        'admin.custom-requests.show' => 'custom_requests.view',
        'admin.custom-requests.edit' => 'custom_requests.edit',
        'admin.custom-requests.update' => 'custom_requests.edit',
        'admin.custom-requests.destroy' => 'custom_requests.delete',
        'admin.custom-requests.submit' => 'custom_requests.create',
        'admin.custom-requests.assign' => 'custom_requests.assign',
        'admin.custom-requests.take' => 'custom_requests.quote',
        'admin.custom-requests.quote' => 'custom_requests.quote',
        'admin.custom-requests.quote.store' => 'custom_requests.quote',
        'admin.custom-requests.quote.update' => 'custom_requests.quote',
        'admin.custom-requests.quote.prepare' => 'custom_requests.quote',
        'admin.custom-requests.quote.send' => 'custom_requests.quote',
        'admin.custom-requests.quote.download' => ['custom_requests.documents', 'custom_requests.view'],
        'admin.custom-requests.request-modification' => 'custom_requests.edit',
        'admin.custom-requests.confirm' => 'custom_requests.confirm',
        'admin.custom-requests.cancel' => 'custom_requests.cancel',
        'admin.custom-requests.documents.store' => 'custom_requests.documents',
        'admin.custom-requests.documents.destroy' => 'custom_requests.documents',
        'admin.custom-requests.comments.store' => 'custom_requests.view',
        'admin.finance.commissions' => 'commissions.view-all',
        'admin.finance.commissions.show' => 'commissions.view-all',
        'admin.finance.commissions.confirm' => 'commissions.manage',
        'admin.finance.commissions.payable' => 'commissions.manage',
        'admin.finance.commissions.paid' => 'commissions.mark-paid',
        'admin.finance.commissions.cancel' => 'commissions.manage',
        'admin.finance.commissions.reverse' => 'commissions.manage',
        'admin.finance.commissions.adjust' => 'commissions.manage',
        'admin.finance.commissions.export.excel' => 'commissions.export',
        'admin.finance.commissions.export.pdf' => 'commissions.export',
        'admin.finance.departures.index' => 'departures_finance.view',
        'admin.finance.departures.show' => 'departures_finance.view',
        'admin.finance.departures.charges.create' => 'departures_finance.manage_charges',
        'admin.finance.departures.charges.store' => 'departures_finance.manage_charges',
        'admin.finance.departures.charges.edit' => 'departures_finance.manage_charges',
        'admin.finance.departures.charges.update' => 'departures_finance.manage_charges',
        'admin.finance.departures.charges.destroy' => 'departures_finance.manage_charges',
        'admin.finance.departures.charges.attachment' => 'departures_finance.view',
        'admin.finance.departures.pdf' => 'departures_finance.export',
        'admin.finance.departures.print' => 'departures_finance.export',
        'admin.finance.departures.excel' => 'departures_finance.export',
        'admin.settings.charge-types.index' => 'charge_types.manage',
        'admin.settings.charge-types.store' => 'charge_types.manage',
        'admin.settings.charge-types.update' => 'charge_types.manage',
        'admin.settings.charge-types.destroy' => 'charge_types.manage',

        'admin.reservation-dossiers.destroy' => 'reservations.delete',

        // Réseau d'agences (branches) : mêmes droits que les points de vente.
        'admin.branches.index' => ['agencies.view', 'points_of_sale.view'],
        'admin.branches.create' => ['agencies.create', 'points_of_sale.create'],
        'admin.branches.store' => ['agencies.create', 'points_of_sale.create'],
        'admin.branches.edit' => ['agencies.edit', 'points_of_sale.edit'],
        'admin.branches.update' => ['agencies.edit', 'points_of_sale.edit'],
        'admin.branches.destroy' => ['agencies.delete', 'points_of_sale.delete'],

        // Médias du studio voyages.
        'admin.local-media.upload' => 'circuits.voyages.view',
    ],

    'route_prefix_permissions' => [
        'admin.dashboard.' => 'dashboard.view',
        'admin.reservations.' => 'reservations.view',
        'admin.custom-requests.' => 'custom_requests.view',
        'admin.assignments.' => 'assignments.view',
        'admin.customers.' => 'customers.view',
        'admin.agent.commissions.' => ['commissions.view-own', 'commissions.view-team', 'commissions.view-all'],
        'admin.agencies.' => ['agencies.view', 'points_of_sale.view'],
        'admin.points-of-sale.' => ['points_of_sale.view', 'agencies.view'],
        'admin.agency-accounts.' => 'agency_accounts.view',
        'admin.agency-employees.' => ['agency_employees.view', 'pos_employees.view'],
        'admin.products.' => 'products.view',
        'admin.circuits.voyages.' => 'circuits.voyages.view',
        'admin.circuits.voyage-themes.' => 'circuits.voyages.view',
        'admin.circuits.taxonomy-terms.' => 'circuits.voyages.view',
        'admin.circuits.airlines.' => 'circuits.voyages.view',
        'admin.circuits.activities.' => 'activities.offers.view',
        'admin.circuits.wp-hotels.' => 'circuits.tour-hotels.view',
        'admin.circuits.tour-hotels.' => 'circuits.tour-hotels.view',
        'admin.circuits.tour-transfers.' => 'transfers.offers.view',
        'admin.accommodations.' => 'accommodations.view',
        'admin.accommodation-packages.' => 'accommodations.packages.view',
        'admin.hajj-omra.requests.' => 'hajj-omra.requests.view',
        'admin.hajj-omra.' => 'hajj-omra.view',
        'admin.economic-offers.requests.' => 'economic-offers.requests.view',
        'admin.economic-offers.' => 'economic-offers.view',
        'admin.activity-offers.' => 'activities.offers.view',
        'admin.wordpress.hotels.' => 'accommodations.wordpress-hotels.view',
        'admin.wordpress.tours.' => 'circuits.voyages.view',
        'admin.wordpress.activities.' => 'activities.offers.view',
        'admin.wordpress.transfers.' => 'transfers.offers.view',
        'admin.operations.' => 'operations.view',
        'admin.visa.' => 'visa.view',
        // Plus specifique que 'admin.finance.' : doit rester declare AVANT lui.
        'admin.finance.control.' => 'finance.projects.view',
        'admin.finance.' => 'finance.view',
        'admin.partners.' => 'partners.view',
        'admin.reporting.' => 'reporting.view',
        'admin.settings.' => 'settings.view',
        'admin.group-deals.trips.' => 'group-deals.trips.view',
        'admin.group-deals.departures.' => 'group-deals.departures.view',
        'admin.group-deals.' => 'group-deals.offers.view',
        'admin.messagerie.' => 'messagerie.view',
        'admin.partner-accounts.' => 'partners.view',
        'admin.partner-commission-rules.' => 'partners.view',
        'admin.wp-media.' => 'circuits.voyages.view',
    ],
];
